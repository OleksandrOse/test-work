 ...
 */
define( 'CUSTOM_API_FOR_WORDPRESS_VERSION', '2.6.0' );
...
function custom_api_wp_get_sql_result( $request ) {
	...
	if ( 'GET' === $need['args'][0]['method'] ) {
		$error_response = array(
			...
			'error_description' => 'Required arguments are missing or do not passed in the correct format.',
		);
...
function custom_api_wp_functions() {
	...
	if ( current_user_can( 'manage_options' ) ) {
		...
		if ( isset( $_POST['mo_custom_feedback_skip'] ) && sanitize_text_field( wp_unslash( $_POST['mo_custom_feedback_skip'] ) ) === 'mo_custom_skip_feedback' ) {
